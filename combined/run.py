...
def move_to_position(desired_pos, mode="direct", max_iterations=100, verbose=False):
    """
    Move the robot's end-effector to the desired Cartesian position using inverse kinematics.
    
    Parameters:
        desired_pos (tuple): The target Cartesian coordinates (x, y, z).
        mode (str): 'direct' for a single IK calculation or 'iterative' for continuous adjustment.
        max_iterations (int): Maximum number of simulation steps to perform.
        verbose (bool): If True, print the current position of the end-effector in each iteration.
    """
    if mode not in ["direct", "iterative"]:
        raise ValueError("Mode must be 'direct' or 'iterative'")

    # Apply inverse kinematics to calculate the joint angles
    if mode == "direct":
...
# Define initial Cartesian positions
initial_positions = [(0.3, 0.3, 0.1), (0.4, 0.3, 0.2), (0.5, 0.5, 0.2)]

# Move to each position using the specified mode
for position in initial_positions:
    move_to_position(position, mode="iterative", max_iterations=100, verbose=True)
